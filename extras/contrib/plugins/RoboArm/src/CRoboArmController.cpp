...
bool CRoboArmController::getPosition(unsigned int& position)
{
	if (!send("COUNTER?\r"))
	{
		return false;
...
bool CRoboArmController::getAngles(unsigned int& angleUp, unsigned int& angleDown)
{
	if (!send("ANGLES?\r"))
	{
		return false;
...
bool CRoboArmController::setAngles(unsigned int angleUp, unsigned int angleDown)
{
...
bool CRoboArmController::continuousMovement(Direction direction, unsigned int speed, unsigned int steps)
{
	if (direction != DIRECTION_DOWN && direction != DIRECTION_UP)
	{
		throw CRoboArmException("Incorrect direction.");
